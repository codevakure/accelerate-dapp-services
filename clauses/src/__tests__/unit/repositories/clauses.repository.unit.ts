...
describe('ClausesRepository (unit)', () => {
  ...
  describe('createClauses', () => {
    it('creates a Clauses', async () => {
      ...
      const result = await clausesRepo.create(aClauses);
  ...
  describe('findClausesById', () => {
    it('returns a clauses if it exists', async () => {
      ...
      expect(await clausesRepo.findById(aClausesWithId.id as string)).to.eql(
        aClausesWithId,
      );
  ...
  describe('findClausess', () => {
    it('returns multiple clausess if they exist', async () => {
      ...
      expect(await clausesRepo.find({})).to.eql(aListOfClausess);
    ...
    it('returns empty list if no clausess exist', async () => {
      ...
      expect(await clausesRepo.find({where: {id: 'hgfhjfghjfjs'}})).to.eql(
        expected,
      );
      sinon.assert.called(find);
    });
  ...
  describe('deleteById()', () => {
    it('successfully deletes existing items', async () => {
      deleteById.resolves();
      await clausesRepo.deleteById(aClausesWithId.id as string);
  ...
  describe('count()', () => {
    it('returns the number of existing clausesLists', async () => {
      ...
      expect(await clausesRepo.count()).to.eql(aListOfClausess.length);
  ...
  describe('updateAll()', () => {
    it('returns a number of clausess updated', async () => {
      ...
      expect(await clausesRepo.updateAll(aChangedClauses, where)).to.eql(1);
